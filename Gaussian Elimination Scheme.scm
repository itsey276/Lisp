...
(define (elimination matrix m n)
  (define (elimination-iter k l)
    (define top-row (maxrow matrix k l))
    (if (or (= k (- m 1)) (= l (- n 1)))
        matrix
        (begin (if (> top-row k)
                   (swap matrix k top-row))
               (if (= (vector-ref matrix (index k l n)) 0)
                   (elimination-iter k (+ l 1))
                   (begin (reduce matrix k l)
                          (elimination-iter (+ k 1) (+ l 1)))))))
    (elimination-iter 0 0))
